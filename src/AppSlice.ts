...
import {Genres, ListMode, MovieDetails, MovieList} from "./Types";
...
export const searchMovie = createAsyncThunk(
    ...
    async (args: { query: string, page: number }) => {
        ...
        const res = await axios
            .get(`${baseUrl}/search/movie?api_key=${movieDbApiKey}&query=${query}&page=${page}`)
            .then((response: AxiosResponse<MovieList>) => response.data)
            .catch((error) => error);
...
export const moviesSlice = createSlice({
    ...
    reducers: {
        ...
        setListMode: (state: MovieSelector, action: PayloadAction<ListMode>) => {
    ...
    extraReducers: {
        ...
        [searchMovie.fulfilled.type]: (state, action) => {
            (state.listOfMovies = {
                ...
                (state.processing = initialState.processing);
            state.listMode = ListMode.SEARCH
